...
obtain_better_sol_heuristica1(Room, Day, AgOpRoomBetter, LAgDoctorsBetter, TFinOp) :-
    get_time(Ti),
    (obtain_better_sol_heuristica1_1(Room, Day); true),
    ...
    get_time(Tf),
    T is Tf - Ti,
    write('Time to generate the solution: '), write(T), nl.

obtain_better_sol_heuristica1_1(Room, Day) :-
    asserta(better_sol(Day, Room, _, _, 1441)),
    findall(OpCode, surgery_id(OpCode, _), LOC),
    prioritize_surgeries(LOC, Day, PriorityLOpCode),
    write('Priority surgery codes: '), write(PriorityLOpCode), nl,
    retractall(agenda_staff1(_, _, _)),
    retractall(agenda_operation_room1(_, _, _)),
    retractall(availability(_, _, _)),
    findall(_, (agenda_staff(D, Day, Agenda), assertz(agenda_staff1(D, Day, Agenda))), _),
    ...
    availability_all_surgeries(PriorityLOpCode, Room, Day),
    agenda_operation_room1(Room, Day, AgendaR),
    update_better_sol_heuristica1(Day, Room, AgendaR, PriorityLOpCode),
    fail.


prioritize_surgeries(LOC, Day, PriorityLOpCode) :-
    findall((OpCode, Doctor, StartTime, Duration, AvailableTime), (
        member(OpCode, LOC),
        surgery_duration(OpCode, Duration),
        assignment_surgery(OpCode, Doctor),
        agenda_staff(Doctor, Day, OccupiedAgenda),
        free_agenda0(OccupiedAgenda, FreeAgenda),
        adapt_timetable(Doctor, Day, FreeAgenda, FreeAgenda1),
        member((StartTime, EndTime), FreeAgenda1),
        AvailableTime is EndTime - StartTime,
        AvailableTime >= Duration
    ), SurgeryOptions),
    write('SurgeryOptions: '), write(SurgeryOptions), nl,
    predsort(compare_priority, SurgeryOptions, SortedOptions),
    write('Sorted SurgeryOptions: '), write(SortedOptions), nl,
    extract_opcodes(SortedOptions, DuplicateLOpCode),
    list_to_set(DuplicateLOpCode, PriorityLOpCode).

compare_priority(<, (Op1, Doc1, Start1, Dur1, Avail1), (Op2, Doc2, Start2, Dur2, Avail2)) :-
    (Dur1 < Dur2 ;
    (Dur1 =:= Dur2, Avail1 < Avail2) ;
    (Dur1 =:= Dur2, Avail1 =:= Avail2, Start1 < Start2) ;
    (Dur1 =:= Dur2, Avail1 =:= Avail2, Start1 =:= Start2, Op1 @< Op2)).
compare_priority(>, (Op1, Doc1, Start1, Dur1, Avail1), (Op2, Doc2, Start2, Dur2, Avail2)) :-
    (Dur1 > Dur2 ;
    (Dur1 =:= Dur2, Avail1 > Avail2) ;
    (Dur1 =:= Dur2, Avail1 =:= Avail2, Start1 > Start2) ;
...
extract_opcodes([], []).
extract_opcodes([(OpCode, _, _)|T], [OpCode|OpCodes]) :-
    extract_opcodes(T, OpCodes).

update_better_sol_heuristica1(Day, Room, Agenda, LOpCode) :-
    better_sol(Day, Room, _, _, FinTime),
    reverse(Agenda, AgendaR),
    write('Agenda before reversing: '), write(Agenda), nl,
    write('Agenda after reversing: '), write(AgendaR), nl,
    evaluate_final_time(AgendaR, LOpCode, FinTime1),
    write('Current FinTime1='), write(FinTime1), nl,
    write('Current FinTime='), write(FinTime), nl,
    FinTime1 < FinTime,
